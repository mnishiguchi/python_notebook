...
def filterStories(stories, triggerlist):
    """
    Params:  a list of news stories and a list of triggers.
    Assume the list of stories is unique - that is, do not include any duplicates in the list.
    Returns: a list of only the stories for which any of the triggers fires on.
    """
    filteredStories = []
    # iterate over stories(type list)
    for story in stories:
        # apply triggers
        for trigger in triggerlist:
            # if a trigger fires, add that story to list
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    and the list of parameters to the constructor,
    and adds a new trigger to the trigger map dictionary.

    arg0 triggerMap:    dictionary with names as keys (strings) and triggers as values
    arg1 triggerType:    string indicating the type of trigger to make (ex: "TITLE")
    arg2 params:    list of strings with the inputs to the trigger constructor (ex: ["world"])
    arg3 name:    a string representing the name of the new trigger (ex: "t1")

    Modifies triggerMap, adding a new key-value pair for this trigger.

    Returns a new instance of a trigger (ex: TitleTrigger, AndTrigger).
    """
    # params:  triggerMap, triggerType, params, name
    
    # create a trigger object according to the triggerType (TITLE, SUBJECT, or SUMMARY)
    if triggerType == 'TITLE':
        tr = TitleTrigger( ''.join(params) )
    elif triggerType == 'SUBJECT':
        tr = SubjectTrigger( ''.join(params) )
    elif triggerType == 'SUMMARY':
        tr = SummaryTrigger( ''.join(params) )
    elif triggerType == 'PHRASE':
        tr = PhraseTrigger( ' '.join(params) )
    elif triggerType == 'NOT':
        tr = NotTrigger( triggerMap[ ''.join(params) ] )       
    elif triggerType == 'AND':
        tr = AndTrigger( triggerMap[ params[0] ] , triggerMap[ params[1] ] )
    elif triggerType == 'OR':
        tr = OrTrigger( triggerMap[ params[0] ] , triggerMap[ params[1] ] )
    else:
        return None
    triggerMap[name] = tr
    return tr
...
def readTriggerConfig(filename):
    ...
    triggers = []    # a list of pointers to a triggerMap object
    triggerMap = {}
    for line in lines:
...
def main_thread(master):
    try:
        ...
        triggerlist = readTriggerConfig("C:\\Users\\Masatoshi_2\\Canopy\\MIT6001x\\week7\\pset7\\myTriggers.txt")
